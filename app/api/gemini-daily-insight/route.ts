...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!journalId || !userId || !journalContent || !emotion || !weatherData) {
    ...
    // Batasi panjang konten jurnal untuk prompt agar tidak terlalu panjang
...
Saat ini, cuaca di ${locationName || 'lokasinya'} adalah '${weatherCondition}' dan kualitas udara '${airQualityStatus}'.

Berikan sebuah paragraf singkat (2-4 kalimat) yang empatik dan suportif.
Hubungkan secara halus bagaimana perasaan pengguna ('${emotion}') mungkin berkaitan dengan kondisi lingkungan (cuaca dan kualitas udara) saat ini.
...
Contoh output yang diinginkan: "Saya memahami kamu merasa [emosi] hari ini. [Komentar singkat menghubungkan emosi dengan cuaca/AQI]. Mungkin [saran aktivitas] bisa membantu menenangkan pikiranmu."
`;
